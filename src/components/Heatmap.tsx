...
const HeatmapText = styled.Text<IActive>`
    ...
    color: ${(props) =>
        props.isSelected ? 'white' : props.activeDays?.indexOf(props.index) > -1 ? 'white' : '#cccccc'};
`;
...
const Touchable = styled(RNBounceable)<IActive>`
    ...
    background-color: ${(props) =>
        props.isSelected
            ...
            ? '#ebd0d1'
            : 'white'};
    border-width: ${(props) => (props.isSelected ? 0 : props.activeDays?.indexOf(props.index) > -1 ? 0 : 0.7)}px;
    border-color: ${(props) =>
        props.isSelected
            ? 'transparent'
            : props.activeDays?.indexOf(props.index) > -1
...
export default ({ data, toDay }) => {
    const [selectedIndex, setSelectedIndex] = useState<number>(moment(toDay).date() - 1);
    const activeDays = [];
        ...
        <>
            <Container>
                ...
                {monthDates.map((_, index) => {
                    if (vacationDays?.includes(index) && selectedIndex !== index) {
                        return (
                            <Touchable
                                key={index}
                        ...
                        return (
                            <Touchable
                                ...
                                isSelected={index == selectedIndex}
                                onPress={() => setSelectedIndex(index)}>
                                <HeatmapText activeDays={activeDays} index={index} isSelected={index == selectedIndex}>
                                    {index + 1}
                                </HeatmapText>
                            </Touchable>
                        );
                ...
                {data.WORKING[selectedIndex][0] > 0 ? (
                    <Row style={{ justifyContent: 'space-around' }}>
                        <IconContainer>
                            ...
                            {moment() > moment(toDay).startOf('month').add(selectedIndex, 'days') &&
