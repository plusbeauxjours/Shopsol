...
export default () => {
    ...
    // const [totalWORKING_COUNT, setTotalWORKING_COUNT] = useState<number>(0); //매장의 전체직원 월간 근무시간(4월24일 적용한 근무시간 계산 로직이 필요함)
    ...
    const prevMonth = () => {
        ...
        fetchSchedulesData(moment(toDay).subtract(1, 'months').format('YYYY-MM-DD'));
    };
    ...
    const init = (CALENDAR_DATA, date) => {
        const currentMoment = moment(date).startOf('month');
        const endMoment = moment(date).endOf('month');
        ...
        const startSubMoment = moment(date).startOf('month');
        const currentSubMoment =
            moment().format('YYYY-MM-DD') == moment(date).format('YYYY-MM-DD') ? moment() : moment(date).endOf('month');
        ...
        try {
            ...
            EMPLOYEE_LIST?.workinglist?.map((i, index) => {
                const WORKING = Array.from(Array(Number(moment(date).endOf('month').date())), () => [
                    0, // 0: 근무시간
                    '00:00', // 1: 시작시간
                    '00:00', // 2: 종료시간
                    false, // 3: 휴가
                                      ...
                                      (i?.jigark === '1' //출근시간~근무종료시간 익일
                                          ...
                                          ? i.UPDATED_END
                                              ...
                                              : (emp.TOTAL_WORKING =
                                                    ...
                                                    Number(+i.REST_TIME * 60000))
                                          : (emp.TOTAL_WORKING = //근무시작시간~근무종료시간 익일
                                                emp.TOTAL_WORKING +
                                                Number(
                                                    moment(i.CHANGE_END, 'kk:mm')
                                                        .add(1, 'days')
