...
export default () => {
    ...
    // const [totalWORKING_COUNT, setTotalWORKING_COUNT] = useState<number>(0);  //매장의 전체직원 주간 근무시간(4월24일 적용한 근무시간 계산 로직이 필요함)
    ...
    // 예를 들면 리덕스에 3월과 4월의 데이터가 있을 때 2021.04.26 ~ 2021.05.02의 주로 이동
    ...
    const init = (CALENDAR_DATA, date) => {
        const currentMoment = moment(date).startOf('isoWeek');
        const endMoment = moment(date).endOf('isoWeek');
        ...
        const startSubMoment = moment(date).startOf('isoWeek');
        const currentSubMoment = moment(date).endOf('isoWeek');
        ...
        try {
                ...
                CALENDAR_DATA[date]?.map((i) => {
                    ...
                    if (emp) {
                        ...
                        ((i.CHANGE_START && i.CHANGE_END) ||
                            ...
                            (i.START && i.END)) &&
                            // setTotalSUB_WORKING_EMP((totalSUB_WORKING_EMP) => totalSUB_WORKING_EMP + 1);

                        setTotalEARLY_COUNT((totalEARLY_COUNT) => totalEARLY_COUNT + (i.alear === '1' ? 1 : 0));

                        setTotalLATE_COUNT((totalLATE_COUNT) => totalLATE_COUNT + (i.jigark === '1' ? 1 : 0));

                        setTotalVACATION_COUNT(
                            (totalVACATION_COUNT) => totalVACATION_COUNT + (i.VACATION == '1' ? 1 : 0),
                        );

                        setTotalNOWORK_COUNT((totalNOWORK_COUNT) => totalNOWORK_COUNT + (i.nowork == '1' ? 1 : 0));
                    }
                });
            });
            ...
            weekDates.map((date, index) => {
                ...
                CALENDAR_DATA[date]?.map((i) => {
                    const emp = empListTemp?.find((j) => j.EMP_SEQ == i.EMP_ID);
                    setTotalREST_TIME_COUNT((totalREST_TIME_COUNT) => totalREST_TIME_COUNT + Number(i.REST_TIME));
                    if (emp) {
                            ...
                            ? ((emp.WORKING[index][10] = i.START_TIME || '미출근'),
                              (emp.WORKING[index][11] =
                                  i.START_TIME && i.AUTOWORKOFF == '1' && !i.END_TIME
                                      ...
                                      ? i.END_TIME
                                      : '미퇴근'))
                            : ((emp.WORKING[index][10] = !i.UPDATED_START ? '미출근' : i.UPDATED_START),
                              (emp.WORKING[index][11] =
                        ...
                        i.CHANGE_START && i.CHANGE_END
                                      ...
                                      (i?.jigark === '1' //출근시간~근무종료시간
                                          ...
                                          ? i.UPDATED_END
                                              ...
                                              : (emp.TOTAL_WORKING =
                                                    ...
                                                    Number(+i.REST_TIME * 60000))
                                          : (emp.TOTAL_WORKING = //근무시작시간~근무종료시간
                                                emp.TOTAL_WORKING +
                                                Number(
                                                    moment(i.CHANGE_END, 'kk:mm')
                                                        .add(1, 'days')
                            ...
                            ? i?.VACATION === '1' && i?.VACATION_PAID === '1'
                                ? ((emp.WORKING[index][0] = -1),
                                  (emp.WORKING[index][7] = true),
                                  (emp.WORKING[index][8] = Number(
                                      moment(i.END, 'kk:mm').add(1, 'days').diff(moment(i.START, 'kk:mm')),
                                  )),
                                  (emp.TOTAL_VACATION_TIME =
                                      emp.TOTAL_VACATION_TIME +
                            ...
                            ? ((emp.WORKING[index][0] = -1),
                              (emp.WORKING[index][7] = true),
                              (emp.WORKING[index][8] = Number(moment(i.END, 'kk:mm').diff(moment(i.START, 'kk:mm')))),
                              (emp.TOTAL_VACATION_TIME =
                                  emp.TOTAL_VACATION_TIME +
                            ...
                            : ((emp.WORKING[index][1] = i.START),
                              ...
                              i?.jigark === '1' //출근시간~근무종료시간
                                  ? i.UPDATED_START
                                      ? (emp.TOTAL_WORKING =
                                            emp.TOTAL_WORKING +
                                            Number(moment(i.END, 'kk:mm').diff(moment(i.UPDATED_START, 'kk:mm'))) -
                                            Number(+i.REST_TIME * 60000))
                                      : (emp.TOTAL_WORKING =
                                            emp.TOTAL_WORKING +
                                            Number(moment(i.END, 'kk:mm').diff(moment(i.START_TIME, 'kk:mm'))) -
                                            Number(+i.REST_TIME * 60000))
                                  : i?.alear === '1' //근무시작시간~퇴근시간
                                  ? i.UPDATED_END
                                      ? (emp.TOTAL_WORKING =
                                            emp.TOTAL_WORKING +
                                            Number(moment(i.UPDATED_END, 'kk:mm').diff(moment(i.START, 'kk:mm'))) -
                                            Number(+i.REST_TIME * 60000))
                                      : (emp.TOTAL_WORKING =
                                            emp.TOTAL_WORKING +
                                            Number(moment(i.END_TIME, 'kk:mm').diff(moment(i.START, 'kk:mm'))) -
                                            Number(+i.REST_TIME * 60000))
                                  : (emp.TOTAL_WORKING =
                                        emp.TOTAL_WORKING +
                                        Number(moment(i.END, 'kk:mm').diff(moment(i.START, 'kk:mm'))) -
                              ...
                              (emp.WORKING[index][0] = Number(moment(i.END, 'kk:mm').diff(moment(i.START, 'kk:mm')))),
                              // setTotalWORKING_COUNT(
                              //     (totalWORKING_COUNT) =>
                              //         totalWORKING_COUNT +
                              //         Number(moment(i.END, 'kk:mm').diff(moment(i.START, 'kk:mm'))),
            ...
            setTotalVACATION_EMP(empListTemp?.filter((i) => i.TOTAL_VACATION > 0).length);
            setTotalNOWORK_EMP(empListTemp?.filter((i) => i.TOTAL_NOWORK > 0).length);

            // 상위직원3명 & 모달의 직원 순서를 위한 소팅
            const orderByWORKING = [...empListTemp];
            setEMP_LIST(orderByWORKING?.sort((a, b) => b.TOTAL_WORKING - a.TOTAL_WORKING));

            const orderByEARLY = [...empListTemp];
            setEARLY_EMP_LIST(orderByEARLY?.sort((a, b) => b.TOTAL_EARLY - a.TOTAL_EARLY));
            ...
            setVACATION_EMP_LIST(orderByVACATION?.sort((a, b) => b.TOTAL_VACATION - a.TOTAL_VACATION));

            const orderByREST_TIME = [...empListTemp];
            setREST_TIME_EMP_LIST(orderByREST_TIME?.sort((a, b) => Number(b.REST_TIME) - Number(a.REST_TIME)));

            const orderByNOWORK = [...empListTemp];
            setNOWORK_EMP_LIST(orderByNOWORK?.sort((a, b) => b.TOTAL_NOWORK - a.TOTAL_NOWORK));
            setLoading(false);
        } catch (e) {
            console.log(e);
        }
